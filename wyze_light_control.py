...
def resolve_runtime_settings(args: argparse.Namespace) -> tuple[dict, dict]:
    ...
    settings = {
        ...
        "sv": resolve_setting(args.sv, "WYZE_SV", config, "sv", DEFAULT_SV),
...
def make_runtime_args(base_args: argparse.Namespace, command: str, value: int | None = None) -> argparse.Namespace:
...
def perform_command(args: argparse.Namespace) -> tuple[int, str, dict]:
    ...
    runtime_args = make_runtime_args(args, args.command, args.value)
    ...
    runtime_args.sv = settings["sv"]

    payload = build_payload(runtime_args, settings["access_token"], settings["phone_id"])
...
def main() -> int:
    ...
    try:
        ...
        args.sc = settings["sc"]
        args.sv = settings["sv"]
        payload = build_payload(args, settings["access_token"], settings["phone_id"])
